...
D_GOAL = 0.3  # The distance(m) at which to switch from conical to constant goal attraction
D_OBS = 0.8  # 1.2 #1.2 # The influence distance(m) of obstacles
...
KMAX = 30  # Maximum magnitude of perpendicular-to-repulsive forces
TAU = 20  # Tau in 2 * KMAX / (1 - e^(delta/tau))
...
ARRIVED_GOAL_DISTANCE = 0.22  # The threshold distance(m) that destination is considered reached
M_PI = 3.141592  # pi
MIN_OMEGA = 0.1  # Minimum rotational velocity
MAX_OMEGA = 0.7
ANGLE_WINDOW = 0.15  # Force angle beyond which we will turn instead of go straight
MAX_VEL = 0.20  # Max allowed forward velocity
OBS_CACHE_TIMEOUT = 3  # Num seconds after which obstacles leave the cache (unless seen again)
OBS_MATCH_DIST = 0.4  # Threshold distance for matching an obstacle in the cache
...
class obstacle_avoidance:
    ...
    def __init__(self):
        ...
        self.publishers['commandVelocity'] = rospy.Publisher('handle_twist', Twist)
    ...
    def findLocalMinima(points):
        ...
        for local in localMinima:
            print
            'minima{} = ({}, {})'.format(i, local.d, local.a)
            i = i + 1
        return localMinima
        ...
        for point in points[1:]:
            if point.d >= 0:  # true iff point is valid
                if last.d >= 0:  # true iff there is an object in progress
                    if abs(point.d - last.d) < 0.9:  # true iff point belongs to the current object
                        if point.d < objMin.d:
                            # if this point is closer than object min, save it
                            objMin = point
                    else:
                        print(abs(point.d - last.d))
    ...
    def navigate(self, scan):
        ...
        if delta < .2:
            self.distanceTraveled += sqrt(
                (self.previousRobotPose.x - self.pose.x) ** 2 + (self.previousRobotPose.y - self.pose.y) ** 2)
        if self.recovering:
    ...
    # def convertRobotCoorToGlobalCoor_with_shifted_origin(self, polarPoint):
    #     # sp = Pose()
    #     # sp.x = round(polarPoint.d * cos(self.pose.theta))
    #     # sp.y = round(polarPoint.d * sin(self.pose.theta))
    #     # return sp
    #     return self.convertRobotCoorToGlobalCoor(polarPoint)
    ...
    def commandTransform(self, initialCommand):
        ...
        if initialCommand.a > ANGLE_WINDOW:  # Dont try to go fwd if the angle is more then fixed value
            ...
            if initialCommand.a < M_PI / 2.0:
                ...
                command.a = MIN_OMEGA + (MAX_OMEGA - MIN_OMEGA) * (initialCommand.a - ANGLE_WINDOW) / (
                            M_PI / 2 - ANGLE_WINDOW)
        elif initialCommand.a < -ANGLE_WINDOW:
            command.a = -MAX_OMEGA
            if initialCommand.a > - M_PI / 2.0:
                command.d = initialCommand.d * cos(initialCommand.a)
                command.a = -MIN_OMEGA - (MAX_OMEGA - MIN_OMEGA) * (ANGLE_WINDOW - initialCommand.a) / (
                            M_PI / 2 + ANGLE_WINDOW)
        else:
    ...
    def calculateNetForce(self, goalWRTrobot, time):
        ...
        for cachedPoint in self.cachedObstacles:
            ...
            rospy.loginfo(
                '[DEBUG] Obj({})PerpF:\t{:.2f}, {:.2f} Theta:\t{:.2f}'.format(objIndex, perpF[0][0], perpF[1][0],
                                                                              theta))
    ...
    def updateObstacleList(self, objects):
        ...
        for obj in self.cachedObstacles:
            ...
            while objRelativeAngle <= 0:
                objRelativeAngle += 2 * M_PI
            rospy.loginfo('[DEBUG] APF, Obj({}) Distance:\t{:.2f} Angle:\t{:.3f}'.format(++objIndex, objDistance,
                                                                                         objRelativeAngle))
        ...
        for pointWRTrobot in objects:
            if pointWRTrobot.d <= D_OBS:
                ...
                rospy.loginfo('[DEBUG] Polar Object that might be added:\t({:.2f}, {:.2f})'.format(pointWRTrobot.d,
                                                                                                   pointWRTrobot.a))
                rospy.loginfo(
                    '[DEBUG] Object in global coor:\t({:.2f}, {:.2f})'.format(pointWRTglobal.x, pointWRTglobal.y))
